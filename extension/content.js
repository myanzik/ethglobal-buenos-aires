...
(function() {
  ...
  function addSponsorButton() {
    ...
    if (issuesHeader) {
      ...
      for (const btn of buttons) {
        ...
        if (text === 'New issue' || text === 'New Issue' || 
    ...
    if (!newIssueButton) {
      ...
      for (const btn of allButtons) {
        ...
        if (text === 'New issue' || text === 'New Issue' || 
          ...
          if (rect.top < 800 && rect.top > 0) { // Within first 800px from top
            newIssueButton = btn;
            targetContainer = btn.parentElement;
            console.log('GitHub Bounty: Found New issue button by text/aria-label');
            break;
          }
        }
      }
    ...
    if (!newIssueButton) {
      const newIssueLinks = document.querySelectorAll('a[href*="/issues/new"], button[onclick*="issues/new"]');
    ...
    if (!newIssueButton) {
      console.warn('GitHub Bounty: New issue button not found. Current URL:', window.location.href);
      console.warn('GitHub Bounty: Available buttons:', Array.from(document.querySelectorAll('button, a[role="button"]')).slice(0, 10).map(b => ({
    ...
    if (newIssueButton.nextSibling) {
      newIssueButton.parentNode.insertBefore(sponsorButton, newIssueButton.nextSibling);
    } else {
      newIssueButton.parentNode.appendChild(sponsorButton);
    }
    
    console.log('GitHub Bounty: Sponsor button added successfully next to New issue button!');
  }
  ...
  async function isMetaMaskInstalled() {
    try {
      ...
      return new Promise((resolve) => {
        const handler = (event) => {
          if (event.data && event.data.type === 'GITHUB_BOUNTY_METAMASK_CHECK') {
            console.log('GitHub Bounty: MetaMask check result:', event.data.isInstalled);
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        const handler = (event) => {
          ...
          if (event.data && event.data.type === 'GITHUB_BOUNTY_METAMASK_CONNECTED') {
            console.log('GitHub Bounty: MetaMask connected, account:', event.data.account);
            window.removeEventListener('message', handler);
            resolve(event.data.account);
          } else if (event.data && event.data.type === 'GITHUB_BOUNTY_METAMASK_ERROR') {
        ...
        // Send message to injected script
        console.log('GitHub Bounty: Sending CONNECT_METAMASK message with chainId:', CONFIG.chainId);
        window.postMessage({
        ...
        setTimeout(() => {
          window.removeEventListener('message', handler);
          reject(new Error('Connection timeout - MetaMask popup may not have appeared'));
        }, 30000);
      });
    } catch (error) {
      console.error('GitHub Bounty: Error ensuring injected script:', error);
      ...
      const handler = (event) => {
        if (event.data && event.data.type === 'GITHUB_BOUNTY_BALANCE_RESULT') {
          ...
          if (event.data.error) {
            ...
            resolve({ balance: event.data.balance, decimals: event.data.decimals });
          }
        }
      };
      window.addEventListener('message', handler);
      ...
      const handler = (event) => {
        if (event.data && event.data.type === 'GITHUB_BOUNTY_SEND_RESULT') {
          ...
          if (event.data.error) {
            ...
            resolve({ success: true, txHash: event.data.txHash, receipt: event.data.receipt });
          }
        }
      };
      window.addEventListener('message', handler);
  ...
  function showSponsorModal(issueData) {
    ...
    connectBtn.addEventListener('click', async () => {
      try {
        ...
        statusEl.textContent = `Connected: ${connectedAccount.substring(0, 6)}...${connectedAccount.substring(38)}`;
        statusEl.className = 'github-bounty-status github-bounty-status-success';
        ...
        try {
          ...
          const balanceFormatted = fractionalPart === 0n 
            ? wholePart.toString() 
    ...
    sendBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        statusEl.innerHTML = `Success! Transaction: <a href="https://sepolia.etherscan.io/tx/${result.txHash}" target="_blank">${result.txHash.substring(0, 10)}...</a>`;
        statusEl.className = 'github-bounty-status github-bounty-status-success';
  ...
  function handleSponsorClick() {
    ...
    if (issueMatch) {
      ...
      const listMatch = issueUrl.match(/github\.com\/([^\/]+)\/([^\/]+)\/issues/);
      if (listMatch) {
