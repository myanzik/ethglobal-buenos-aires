...
contract RewardDistributor {
    ...
    function fundIssue(
        ...
        bytes32 issueId = issueTracker.generateIssueId(owner, repo, issueNumber);
        
        // Check if issue exists, register if not
        (,, uint256 existingIssueNumber,,,) = issueTracker.getIssue(issueId);
    ...
    function distributeReward(
        ...
        require(issueTracker.isContributor(issueId, contributor), "Not a contributor");
        
        // Check if issue is closed
        (,,, uint256 totalFunding, bool isClosed,) = issueTracker.getIssue(issueId);
        ...
        require(distributedRewards[issueId][contributor] == 0, "Reward already distributed");
        
        // Check contract has enough funds
        require(issueFunds[issueId] >= reward, "Insufficient funds in contract");
     ...
     */
    function getTotalPendingRewards(bytes32 issueId) public view returns (uint256) {
        address[] memory contributors = issueTracker.getContributors(issueId);
        uint256 total = 0;
     ...
     */
    function areAllRewardsDistributed(bytes32 issueId) public view returns (bool) {
        address[] memory contributors = issueTracker.getContributors(issueId);
